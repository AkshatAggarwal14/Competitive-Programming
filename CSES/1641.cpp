...
// Created at: 02/08/2021 09:25:49 (UTC +5:30)
//
...
void Solution() {
    ...
    fo(i, n - 1) {
        rep(j, i + 1, n) {
            ll left = x - a[i].ff - a[j].ff;
            debug(left);
            if (cnt.count(left) && a[i].ss != a[j].ss) {
                //use binary search to find for left;
                ll l = j, r = n;
                while (r > l + 1) {
                    ll m = (l + r) / 2;
                    if (a[m].ff <= left) {
                ...
                debug(i, j, l, a[i].ff, a[j].ff, a[l].ff);
                if (l != i && l != j)
                    cout << a[i].ss << " " << a[j].ss << " " << a[l].ss;
                else
                    continue;
                return;
            }

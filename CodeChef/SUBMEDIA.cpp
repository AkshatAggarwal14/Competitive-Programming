...
//     });
//     cout << maxx << nline;
//     for (auto val : dq) cout << val.ff << space;
// }
...
void Solution() {
    ...
    cin >> n >> k;
    vpl a(n);
    fo(i, n) cin >> a[i].ff, a[i].ss = i;
    sort(all(a));
    vpl vals;
    for (ll i = n - 1; i >= n - k; --i) vals.eb(a[i]);
    // dbg(vals);
    sort(all(vals));
    dbg((k - 1) / 2, vals);
    print(vals[(k - 1) / 2].ff);
    sort(all(vals), [&](pl A, pl B) { return A.ss < B.ss; });

    fo(i, sz(vals)) cout << vals[i].ff << " ";
    cout << ln;
}

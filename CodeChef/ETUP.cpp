...
void Solution() {
    ll n, q, temp;
    cin >> n >> q;
    vl pre_even(n + 1, 0), pre_odd(n + 1, 0);
    rep(i, 1, n + 1) {
        cin >> temp;
        if (temp & 1) {
            pre_odd[i] = pre_odd[i - 1] + 1;
            pre_even[i] = pre_even[i - 1];
        } else {
            pre_odd[i] = pre_odd[i - 1];
            pre_even[i] = pre_even[i - 1] + 1;
        }
    }
    ll l, r;
    while (q--) {
        cin >> l >> r;
        ll e = pre_even[r] - pre_even[l - 1];
        ll o = pre_odd[r] - pre_odd[l - 1];
        ll Ans = 0;
        if (e > 2) {
            Ans += (e * (e - 1) * (e - 2)) / 6;
        }

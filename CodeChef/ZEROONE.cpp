...
void Solution() {
    ...
    cin >> n;
    vector<ll> zero, one;
    for (ll i = 0; i < n; ++i) cin >> num, (i & 1) ? (one.push_back(num)) : (zero.push_back(num));
    sort(all(zero), greater<ll>());
    sort(all(one), greater<ll>());
    vector<ll> suff(n / 2);
    partial_sum(all(one), suff.begin());
    reverse(all(suff));
    reverse(all(one));
    ll ans = 0, pos1 = 0, pos2 = 0;
    for (ll i = 0; i < n / 2; ++i) ans += zero[i] * suff[i];
    for (ll i = 0; i < n; ++i) cout << ((i & 1) ? (one[pos1++]) : (zero[pos2++])) << " \n"[i == n - 1];
    cout << ans << '\n';
}

...
void Solution() {
    ...
    cin >> n;
    safe_map<ll, ll> m;
    fo(i, n) cin >> a >> b, ++m[a + b];
    bool ans = true;
    /*
        /  this type of diagonals cause problems
        if(cnt[sum]>=sum-1) <- NO for / until secondary diag
        if(cnt[sum]>=2*n-sum+1) <- NO for diags after that
    */
    for (auto &it : m)
        if (it.ss > min(it.ff - 2, 2 * n - it.ff)) ans = false;
    cout << (ans ? "YES\n" : "NO\n");
}

...
void solve() {
    ...
    cin >> a >> b;
    // 2^30
    //we can go from MSB to LSB and check which can be set to maximise
    //for each card- undecided, flipped, not flipped
    // -1 , 0 , 1

    //kth bit set or not set
    vl state(n, -1);
    for (ll bit = 1LL << 30; bit > 0; bit >>= 1) {
        bool pos = true;  //assume bit is set
        fo(i, n) {
        ...
        fo(i, n) {
            ...
            if (!(b[i] & bit)) state[i] = 0;
        }
    }
    ll ans = (1LL << 30) - 1, flips = 0;
    for (ll i = 0; i < n; i++) {
        if (state[i] == 1) {
            flips++;
            ans &= b[i];

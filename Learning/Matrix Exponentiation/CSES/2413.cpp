...
// counting towers problem but for large n using matrix exponentiation.
...
int main() {
    ...
    */
    ...
    while (q--) {
        int n;
        cin >> n;
        Matrix ans = power(Transform, n - 1) * Base;
        cout << (ans.mat[0][0] + ans.mat[1][0]) % MOD << '\n';
    }
}
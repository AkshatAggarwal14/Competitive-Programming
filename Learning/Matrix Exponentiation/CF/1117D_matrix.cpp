...
int main() {
    ...
    int m;
    cin >> n >> m;
    // we can either split last gem or not
    // dp[n] = dp[n - m] + dp[n - 1]
    // split it or not split it
    // dp[0] = 1, dp[1] = 1, dp[m - 1] = 1, then use above formula
    // dp[m] = dp[m - 1] + dp[0]
    // [m ... 2 1] = [] * [m-1 m.. 1 0]
    // first row = [1 0... 0 1]
    // second    = [1 0......0]
    Matrix Base(m, m, 0), Transform(m, 1, 1);
    Base[0][0] = Base[0][m - 1] = 1;
    for (int i = 1; i < m; ++i)
        Base[i][i - 1] = 1;
    cout << (power(Base, n - m + 1) * Transform)[0][0] << '\n';
}
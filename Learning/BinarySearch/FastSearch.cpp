...
//so last index whrn arr[l]<x,r=l+1
ll lowerIndex(ll x) {
    ll n = sz(arr);
    ll l = -1, r = n;
    while (r > l + 1) {  //stops when r==l+1
        ll mid = (l + r) / 2;
        if (arr[mid] >= x)  //invariant arr[r] is always >= x
            r = mid;
        else
            l = mid;
    }
...
// so first index r which is >y l=r-1
ll upperIndex(ll y) {
    ll n = sz(arr);
    ll l = -1, r = n;
    while (r > l + 1) {  //stops when r==l+1
        ll mid = (l + r) / 2;
        if (arr[mid] <= y)  //invariant arr[l]<=x
            l = mid;
        else
            r = mid;
    }
...
void Solution() {
    ...
    while (k--) {
        ll l, r;
        cin >> l >> r;
        cout << upperIndex(r) - lowerIndex(l) + 1 << " ";
    }
}

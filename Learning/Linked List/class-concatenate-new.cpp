...
class LL {
   ...
   public:
    ...
    template <class U>
    friend void Concat(LL<U> f, LL<U> s);
...
template <class T>
LL<T>::LL(T A[], int n) {
    Node<T> *t, *last;    //last will be used to enter a new node at the ending
...
int main() {
    ...
    cerr << endl;
    cerr << "\x1B[34m" << (float)clock() / CLOCKS_PER_SEC << " secs\033[0m\t\t" << endl;
    return 0;
}
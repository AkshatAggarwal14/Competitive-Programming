...
void test() {
    ...
    // we need only last 2 states
    dp[0][0] = dp[1][0] = 1;  // sum 0 with 0 elements
    for (int i = 1; i <= n; ++i) {
        for (int j = 1; j <= sum / 2; ++j) {
            dp[i & 1][j] = dp[(i + 1) & 1][j];  // subsets with sum j, without ith element
            if (i <= j)
                (dp[i & 1][j] += dp[(i + 1) & 1][j - i]) %= MOD;  // sum j with ith element
        }
    }

...
void dfs(int i, int parent = -1) {
    ...
    if (largest != -1) {
        // can swap above in loop also, but this is more readable
        swap(subtree[i], subtree[largest]);  // largest sized child
    }

    for (int child : children) {
        if (child == largest) continue;
        if (!subtree[child].empty()) subtree[i].insert(subtree[child].begin(), subtree[child].end());
    }

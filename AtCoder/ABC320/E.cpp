...
void test() {
    ...
    for (ll i = 0; i < m; ++i) {
        ll T, W, S;
        cin >> T >> W >> S;
        auto it = returnTimes.begin();
        while (it != returnTimes.end() and it->first <= T) {
            for (auto &returns : it->second) {
                st.update(returns, 1);
            }
            it = returnTimes.erase(it);
        }
        int id = st.kth_order(1);
        if (id < n) {
            ans[id] += W;
            st.update(id, 0);
            returnTimes[T + S].push_back(id);
        }
    }

...
void Solution() {
    ...
    cin >> pt;
    map<pl, ll> cnt;
    // set<vpl> pts; //can also be used to store rect points
    fo(i, n)++ cnt[pt[i]];
    // for (a,b) && (c,d) check if (a,d) && (c,b) are present
    ll rect = 0;
    fo(i, n) {
        fo(j, n) {
            if (i != j) {
                pl p1 = {pt[i].ff, pt[j].ss}, p2 = {pt[j].ff, pt[i].ss};
                if (p1 != pt[i] && p1 != pt[j] && p2 != pt[i] && p2 != pt[j] && p1 != p2) {
                    if (cnt.count(p1) && cnt.count(p2)) {
                        rect++;  //counted 4 times
                    }
                }
            }
        }

...
bool binary_srch(vector<pair<ll, ll>> &v, ll len) {
    ll n = v.size();
    ll mini = inf, maxi = -1;
    ll left = 0, right = 0;
    while (right < n) {
        while (left <= right && v[right].first - v[left].first >= len) {
            mini = min(mini, v[left].second);
            maxi = max(maxi, v[left].second);

            left++;
        }

        if (maxi != -1 && (abs(v[right].second - mini) >= len || abs(maxi - v[right].second) >= len)) {
            return true;
        }
        right++;
    }
...
void Solution() {
    ...
    cin >> v;
    sort(all(v));
    ll l = 0, r = 1e9 + 1, ans = -1, mid;
    while (l <= r) {
        mid = (r - l) / 2 + l;
        if (binary_srch(v, mid)) {
            l = mid + 1;
            ans = mid;
        } else {
            r = mid - 1;

...
void Solution() {
    ...
    dbg(B, S);
    vpl BB, SS;
    tr(it, B) BB.eb(it->ff, it->ss);
    tr(it, S) SS.eb(it->ff, it->ss);
    // dbg(bb, ss);
    reverse(all(BB));
    reverse(all(SS));

    ll sN = min(s, sz(SS)), bN = min(s, sz(BB));
    // print s of SS from end
    if (sz(SS)) {
        vpl vals(sN);
        reverse(all(SS));
        fo(i, sN) vals.pb(SS[i]);
        reverse(all(vals));
        fo(i, sN) cout << "S " << vals[i] << ln;
    }  // print s of BB from begin
    if (sz(BB))
        fo(i, bN) cout << "B " << BB[i] << ln;
}

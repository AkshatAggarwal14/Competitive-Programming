...
inline ll M(const ll &x) { return ((x % MOD) + MOD) % MOD; }
inline ll add(const ll &x, const ll &y) { return (M(x) + M(y)) % MOD; }
inline ll mul(const ll &x, const ll &y) { return (M(x) * M(y)) % MOD; }
...
ll C(const ll &a, const ll &b) {
    if (a < b) return 0;
    return mul(mul(fact[a], invf[b]), invf[a - b]);
}

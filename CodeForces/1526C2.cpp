...
void Solution() {
    ...
    fo(i, n) {
        if (sum + v[i] >= 0) {
            sum += v[i];
            ans++;
            if (v[i] < 0) s.insert(v[i]);  // take care of -ves seperately
        } else {
            if (s.empty()) continue;  //! element skipped
            long mn = *(s.begin());   // most negative potion
            if (mn < v[i]) {          // meansif current has more magnitude
                sum -= mn;
                sum += v[i];
                s.erase(s.begin());
                s.insert(v[i]);

...
void solve() {
    ...
    int mx = *max_element(all(cnt));                     //count of max
    int ctr = n - count(cnt.begin() + 1, cnt.end(), 0);  //count of non zero
    // for (int i = 1; i <= n; i++) {
    //     ctr += (cnt[i] == 0 ? 0 : 1);
    //     // ctr += !!cnt[i];  //<- converts any count to 1
    // }
    if (mx == ctr)
        cout << min(mx, ctr) - 1;  //min(mx,ctr)-(mx==ctr)
    else
        cout << min(mx, ctr);

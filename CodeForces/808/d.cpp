...
void Solution() {
    ...
    multiset<ll> pre, suff;
    fo(i, n) suff.insert(a[i]);
    ll sum_pre = 0, sum_suff = 0, ele;
    fo(i, n) {
        pre.insert(a[i]);
        suff.erase(suff.find(a[i]));  //Just to erase 1
        sum_pre += a[i];
        sum_suff = sum - sum_pre;
        ele = abs(sum_pre - sum_suff) / 2;
        if (abs(sum_pre - sum_suff) % 2 == 1) continue;
        if (sum_pre > sum_suff) {
            if (present(ele, pre)) {
                yes();
                return;
            }
        } else if (sum_pre < sum_suff) {
            if (present(ele, suff)) {
                yes();
                return;
            }

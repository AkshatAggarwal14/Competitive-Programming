...
void Solution() {
    ...
    for (ll i = 0; i < n; ++i) {
        cin >> x[i];
        ld ans = r;
        // check all previous x's if dist is <= 2 * r they might touch, find y using this from pythagoras theorem then find such max y
        for (ll j = 0; j < i; ++j) {
            if (abs(x[i] - x[j]) > 2 * r) continue;
            auto sq = [](ld X) { return X * X; };
            ld cy = y[j] + sqrtl(sq(2 * r) - sq(x[i] - x[j]));
            amax(ans, cy);
        }
        y[i] = ans;
        cout << ans << ' ';

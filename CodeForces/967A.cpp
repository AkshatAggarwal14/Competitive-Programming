...
void Solution() {
	...
	cin >> n >> s;
	vpl tim(n);
	fo(i, n) cin >> tim[i].ff >> tim[i].ss;
	debug(tim);
	vector<pair<ll, bool>> dur(n - 1, {0, false});
	fo(i, n - 1) {
		dur[i].ff = (tim[i + 1].ff - tim[i].ff) * 60 + (tim[i + 1].ss - tim[i].ss);
		if (dur[i].ff > 2 * s + 1) dur[i].ss = true;
	}
	debug(dur);
	if ((tim[0].ff) * 60 + (tim[0].ss) > s) {
		cout << 0 << " " << 0 << ln;
		return;
	}
	fo(i, n - 1) {
	...
	//if no interval in between
	ll hh = tim[n - 1].ff;
	ll mm = tim[n - 1].ss;
	mm += s + 1;
	if (mm >= 60) hh += mm / 60, mm %= 60;
	cout << hh << " " << mm << ln;
	// sort(all(tim), [&](pl a, pl b) {

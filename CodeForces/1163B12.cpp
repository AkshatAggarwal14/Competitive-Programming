...
void Solution() {
    ...
    rep(i, 1, 1 + n) {
        ...
        if (cnt.count(x)) {
            t[cnt[x]]--;
            if (t[cnt[x]] == 0) t.erase(cnt[x]);  //to reduce size of map
        }
        cnt[x]++;
        t[cnt[x]]++;
        if (t.size() == 1) {
            if (t.begin()->ss == 1 || t.begin()->ff == 1) ans = i;
        } else if (t.size() == 2) {
            vector<pl> v;
            for (auto pr : t) v.eb(pr);
            if (v[0].ss == 1) {
                if (v[0].ff == 1 || v[0].ff == v[1].ff + 1) ans = i;
            }
            if (v[1].ss == 1) {
                if (v[1].ff == 1 || v[1].ff == v[0].ff + 1) ans = i;
            }
        }
    }
    cout << ans << ln;

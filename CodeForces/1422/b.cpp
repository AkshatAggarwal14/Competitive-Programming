...
void Solution() {
    ...
    ll ans = 0;
    for (ll c1 = 0, c2 = m - 1; c1 < c2 + m % 2; c1++, c2--) {
        for (ll r1 = 0, r2 = n - 1; r1 < r2 + n % 2; r1++, r2--) {
            vl temp;
            temp.pb(mat[r1][c1]);
            if (r1 != r2) temp.pb(mat[r2][c1]);
            if (c2 != c1) temp.pb(mat[r1][c2]);
            if (r1 != r2 && c1 != c2) temp.pb(mat[r2][c2]);
            sort(all(temp));
            ll med = temp[sz(temp) / 2];  //! MEDIAN
            fo(i, sz(temp)) ans += abs(med - temp[i]);
        }
    }

...
void Solution() {
    ...
    str s;
    map<char, ll> sing;
    map<pair<char, char>, ll> doub;
    fo(i, n) {
        cin >> s;
        set<char> st;
        fo(j, sz(s)) st.insert(s[j]);
        if (sz(st) >= 3)
            continue;
        else if (sz(st) == 2) {
            char c1 = *(st.begin()), c2 = *(st.rbegin());
            doub[{c1, c2}] += count(all(s), c1) + count(all(s), c2);
        } else {
            sing[*(st.begin())] += count(all(s), *st.begin());
        }
    }
    ll ans = LLONG_MIN;
    for (char a = 'a'; a <= 'z'; ++a) {
        for (char b = 'a'; b <= 'z'; ++b) {
            if (a != b)
                amax(ans, sing[a] + sing[b] + doub[{a, b}]);
            else
                amax(ans, sing[a]);
        }
    }

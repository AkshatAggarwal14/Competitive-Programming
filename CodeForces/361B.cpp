...
void Solution() {
	ll n, k;
	cin >> n >> k;
	if (n == k) { //as 1 1 will always give 1 gcd which will thus ruin the condition
		cout << -1 << ln;
		return;
	}
	//n k  1  2  3  ...   n-k-1 | n-k+1 n-k+2  ...  n
	// this above will make exactly k elements on their actual i thus gcd(a,b)>1 and for other elements gcd(a,a-1)=1
	cout << n - k << " ";
	fo(i, n - k - 1) cout << i + 1 << " ";
	fo(i, k) cout << n - k + 1 + i << " ";
	cout << ln;

...
void Solution() {
    ...
    fo(k, q) {
        ...
        if (type == 1) {
            ...
            cerr << "*before clearing* ";
            debug(vec);
            //because of last noble
            vvl vec2(n);
            fo(i, n) {
                fo(j, n) {
                    if (j > i)
            ...
            debug(vec2);
            for (auto ar : vec2) {
                ll sum = accumulate(all(ar), 0);
                if (sum != 0) {
                    cerr << "dead " << n - sz(ar) << ln;
                    alive.erase(n - sz(ar));
                }
            }
            cout << sz(alive) << ln;

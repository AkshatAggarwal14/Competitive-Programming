...
void Solution() {
    ...
    fo(i, n) {
        ...
        ll cnt = 1;
        if (curr == '1') {
            while (i < n - 1 && s[i + 1] == curr) i++, cnt++;
            // compress.emplace_back(s[i], cnt);
            compressed.push_back(cnt);
        }
    }
    // dbg(compress);
    ll ans = 0;
    sort(all(compressed), greater<>());
    fo(i, sz(compressed)) if (i % 2 == 0) ans += compressed[i];
    cout << ans << '\n';
}

...
void Solution() {
    ...
    fo(i, n) {
        ...
        fo(j, i) {
            freq[a[j]]++;
            if (freq[a[j]] == 2) {
                validPrefix = false;  //so take out index until unique prefix
                break;
            }
        }
        if (!validPrefix) {  // prefix only until no repeating ele
            continue;
        }
        ...
        for (ll j = n - 1; j >= i; --j) {
            ...
            if (freq[a[j]] == 1) {
                min_index_suffix = j;
            } else  //twice in suff or already in pre <-- important
                break;
        }
        amin(ans, min_index_suffix - i);  // i is pre len
    }
    print(ans);
}

...
class InputDevice {
   protected:
    ...
    InputDevice(const char* head1, const char* tail1) : head(head1), tail(tail1), base(setBase().value) {}
   ...
   public:
    ...
    inline typename enable_if<is_integral<I>::value,
                              bool>::type
    read(char* arg, I limit, Terminator terminator, Ts&&... args) {
    ...
    inline typename enable_if<is_iterator<Iterator>::value,
                              bool>::type
    read(Iterator first, Iterator last, Ts&&... args) {
...
class InputFile : public InputDevice {
   ...
   public:
    InputFile(FILE* file1 = stdin, bool lineBuffered1 = true, bool takeOwnership = false)
        : InputDevice(buffer, buffer), file(file1), lineBuffered(lineBuffered1), owner(takeOwnership) {}
...
class OutputDevice {
   protected:
    ...
    OutputDevice() : output(buffer), end(buffer + BUFFER_SIZE + MAX_LENGTH), separate(false), width(setWidth().value), fill(setFill().value), base(setBase().value), precision(setPrecision().value), delimiter(setDelimiter().value) { computeBasePower(); }
    ...
    inline char* writeUnsignedInt(I arg, char* last) {
        ...
        if (__builtin_expect(base == 10, true)) {
            for (; arg; arg /= 10) *--last = '0' + char(arg % 10);
        } else
            for (; arg; arg /= base) {
                I digit = arg % base;
                *--last = digit < 10 ? '0' + char(digit) : 'A' - 10 + char(digit);
            }
        return last;
    }
   ...
   public:
    ...
    inline typename enable_if<is_iterator<Iterator>::value,
                              int>::type
    write(Iterator first, Iterator last, Ts&&... args) {
        int charsWritten = 0;
...
class OutputFile : public OutputDevice {
   ...
   public:
    OutputFile(FILE* file1 = stdout, bool takeOwnership = false) : file(file1), owner(takeOwnership) {}
...
class OutputString : public OutputDevice {
   ...
   public:
    OutputString(string& str1) : OutputDevice(), str(str1) {}

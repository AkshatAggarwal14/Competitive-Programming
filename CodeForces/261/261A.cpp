...
void Solution() {
    ...
    vl a(n);
    fo(i, n) cin >> a[i];
    ll min_q = *min_element(all(q));  //better discount of 2 items on less number of items
    sort(all(a), greater<ll>());
    ll ans = 0;
    fo(i, n) {
        if (i % (min_q + 2) < min_q) {  //0,1,2,... min_q-1
            ans += a[i];                //thus we are buying top q items and including next 2 as free
            dbg(i);
        }
    }
    cout << ans << ln;

...
// Created at: 04/08/2021 00:05:37 (UTC +5:30)
//
...
void Solution() {
    str s;
    cin >> s;
    str x = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz-_";
    map<char, ll> mep;
    fo(i, 64) mep[x[i]] = i;
    vl a(64, 0);
    fo(i, 64) {
        fo(j, 64) {
            fo(k, 64) {
                if ((j & k) == i) {  //counts pairs with bitwise and equal to i in a[i]
                    ++a[i];
                }
            }
        }

...
void solve()
{
    ...
    rep(i, 1, n) if (arr[i] - arr[i - 1] > x)
        ans.pb((arr[i] - arr[i - 1] + 1) / x); //stores differences greater than equal 1*x
    ...
    while (ans.size())
    {
        ...
        k -= ans[ans.size() - 1]; //subtract last element
        ans.pop_back();
    }
    cout << ans.size() + 1; //groups = gaps + 1

...
void Solution() {
    ...
    ll sum = 0;
    fo(i, n) cin >> ab[i].ff;
    fo(i, n) cin >> ab[i].ss, sum += ab[i].ss;
    sort(all(ab));  //sort by courier time
    ll max_courier = -1, ans = min(ab[n - 1].ff, sum);
    fo(i, n) {
        max_courier = ab[i].ff;  //smaller couriers also arrive in this time
        sum -= ab[i].ss;         //time taken to bring all other pickups
        amin(ans, max(sum, max_courier));
    }
    cout << ans << ln;
}

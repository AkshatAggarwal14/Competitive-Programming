...
void Solution() {
    ...
    deque<int> a(n);
    fo(i, n) cin >> a[i];
    // if max reaches position 1 then others will cycle around from 2nd pos to n-1th (period = n-1)
    V<P<int>> same;
    int ctr = 0;
    ...
    // int save = cnt[a];
    // cnt.clear();
    // // cnt[a] is the occurence first time, and period is the period after which it repeats
    // // sz(same) = cnt[a]-1+period
    // fo(i, q) {
    //     long long int query;
    //     cin >> query;
    //     query--;
    //     if (query <= sz(same) - 1)
    //         cout << same[query].first << " " << same[query].second << '\n';
    //     else {
    //         query = (query - sz(same)) % period + save - 1;
    //         cout << same[query].first << " " << same[query].second << '\n';
    //     }
    // }
}

...
void test() {
    ...
    for (auto &x : a) cin >> x;
    SegTree<ll> OR(
        a, [](ll A, ll B) { return A | B; }, []() { return 0LL; });
    SegTree<ll> AND(
        a, [](ll A, ll B) { return A & B; }, []() { return (1LL << 31) - 1; });
    using a32 = array<ll, 32>;
    vector<a32> v(n);
    for (ll i = 0; i < n; ++i) {
        for (ll j = 0; j < 32; ++j) {
            if (a[i] & (1 << j)) {
    ...
    SegTree<a32> SUM(
        ...
        [](a32 A, a32 B) {
        a32 sum;
        for (ll i = 0; i < 32; ++i) sum[i] = A[i] + B[i];
        return sum; },
        []() {
        a32 zeroes;
        fill(zeroes.begin(), zeroes.end(), 0);
        return zeroes; });
    while (q--) {
        int L, R;
        cin >> L >> R, --L, --R;
        ...
        cout << OR.query(L, R) << ' ' << AND.query(L, R) << ' ' << last << '\n';
    }
}

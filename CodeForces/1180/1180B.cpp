...
void Solution() {
    ...
    fo(i, n) {
        cin >> a[i];
        if (a[i] >= 0) a[i] = -a[i] - 1;  //all 0s to -1 and all other positives to negative
    }
    //now all numbers are negative, and max can be -1
    ll min_ind = 0;
    //if odd convert largest to positive
    fo(i, n) if (a[i] < a[min_ind]) min_ind = i;  //find index of smallest negative
    if (n & 1) a[min_ind] = -a[min_ind] - 1;      //-1->0 and other -ve to some pos
    fo(i, n) cout << a[i] << " ";

...
void Solution() {
    ...
    fo(i, n) {
        ...
        while (getline(ss, token, ' ')) {
            if (sz(token) >= 2 && token[sz(token) - 2] == '\'') {
                lines[i].eb(token.substr(0, sz(token) - 2));
            } else
                lines[i].eb(token);
        }
    }
    ...
    dbg(prior);
    vector<pair<str, ll>> cost;
    tr(it, prior) cost.eb(it->ff, it->ss);
    sort(all(cost), [&](pair<str, ll> a, pair<str, ll> b) {
        if (a.ss == b.ss) {
            // if (sz(a.ff) == sz(b.ff)) {
            //     return a.ff < b.ff;
            // }
    ...
    dbg(cost);
    fo(i, sz(cost)) if (cost[i].ff != s1) print(cost[i].ff);
}

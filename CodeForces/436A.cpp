// Problem: A. Feed with Candy
// Contest: Codeforces - Zepto Code Rush 2014
// URL: https://codeforces.com/contest/436/problem/A
// Author: Akshat Aggarwal , @master._.mind , NIT Hamirpur
// Created at: 04/08/2021 11:32:39 (UTC +5:30)
//
...
void Solution() {
    ll n, x;
    cin >> n >> x;
    vector<pair<pl, ll>> vec(n);
    fo(i, n) cin >> vec[i].ff.ff >> vec[i].ff.ss >> vec[i].ss;
    sort(all(vec), [&](pair<pl, ll> a, pair<pl, ll> b) {
        if (a.ss == b.ss) return a.ff.ss < b.ff.ss;
        return a.ss < b.ss;
    });
    debug(vec);
}
